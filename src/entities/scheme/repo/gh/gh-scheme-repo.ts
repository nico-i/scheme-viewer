...
export class GithubSchemeRepo implements SchemeRepo {
	...
	async getSchemesByBase(base: Base): Promise<Scheme[]> {
        ...
        if(this.useCache) {
            ...
            data = await axios
                .get<unknown>(`https://api.github.com/repos/tinted-theming/schemes/contents/${base.name}`)
                .then((res) => {
                    if(this.updateCache) fs.writeFileSync(`cache/${base.name}.json`, JSON.stringify(res.data));
                    return res.data})
                .catch((err) => {
                    console.error(err);
                    return baseCache;
